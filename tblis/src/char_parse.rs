...
pub fn char_parse(indices: &[&str]) -> Vec<Vec<c_char>> {
    ...
    if all_chars.iter().all(|&c| c.is_ascii()) {
        return indices.iter().map(|s| s.chars().map(|c| c as c_char).collect()).collect();
    }
    match all_chars_len {
        0..=62 => {
            // rule 2
            ...
            indices.iter().map(|s| s.chars().map(|c| char_map[&c]).collect()).collect()
        },
        63..=128 => {
            // rule 3
            let char_map = all_chars
        ...
        _ => panic!("Too many unique characters in indices ({all_chars_len} > 128)."),
    }
}
